#define TOPBARHEIGHT 3
#define BUTTON_HEIGHT 3
#define BUTTON_SIDE_OFFSET 2 // how many columns from the edge of menu, should the button be
#define BUTTON_TOP_OFFSET 2
#define CURSORSPEED 9            // must be an odd number !!!!
#define COLUMN_TO_WIDTH_RATION 2 // ration between column and height - approximately one row is the same amount of pixels as two columns
#define NO_ORDER_ASSIGNED_MESS "No order assigned to this table, do you want to create one?"
#define DEFAULT_WIDTH_DEVIDER "--------------------------------------------------------"
#define ORDER_ITEM_VIEW_DIVIDER "------------------------------------"

// pop up menus configurations
#define NUMBER_OF_BUTTONS_IN_SCROLL 5
#define NUMBER_OF_SCROLL_BUTTONS_ORDERS 10
#define NUMBER_OF_SCROLL_BUTTONS_MENU 13
#define NUMBER_OF_BUTTONS_IN_SCROLL_DISCOUNT 4

#define DEFAULT_WIDTH 60
#define DEFAULT_HEIGHT 40

#define WAITER_POP_UP_HEIGHT 25
#define LOCAL_ORDER_POP_UP_HEIGHT 50
#define MENU_POP_UP_HEIGHT 50
#define TEXT_PROMPT_HEIGHT 6
#define NO_ORDER_ASSIGNED_POP_UP_HEIGHT 9
#define ORDERS_POP_UP_HEIGHT 50
#define MENU_ITEM_POP_UP_MENU_HEIGHT 12
#define MENU_ITEM_POP_UP_MENU_WIDTH 40
#define ORDER_ITEM_POP_UP_MENU_HEIGHT 20
#define ORDER_ITEM_POP_UP_MENU_WIDTH 40

#define DEFAULT_TEXT_SECTION_START_Y 1
#define DEFAULT_SCROLL_SECTION_START_Y 4
#define ORDER_SCROLL_SECTION_START_Y 8
#define SELECT_QUANTITY_SCROLL_SECTION_START 3

// TEXT_SECTION_START_Y
// SCROLL_START_Y
//

#define MINIMUM_WINDOW_HEIGHT 80
#define MINIMIUM_WINDOW_WIDTH 200